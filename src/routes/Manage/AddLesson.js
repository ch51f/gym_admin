...
@connect(({loading, manage}) => ({
  submitting_add: loading.effects['manage/addLesson'],
	submitting_up: loading.effects['manage/updateLesson'],
...
export default class Page extends Component {
    ...
    let {form, dispatch} = this.props;
      ...
      if (!err) {
        ...
        if(values.item_id) {
          ...
          dispatch({
            type: 'manage/updateLesson',
            payload: params,
          })
        } else {
          dispatch({
            type: 'manage/addLesson',
            payload: params,
          })
        }
      }
  ...
  callback(img) {
    ...
    const submitting = lesson.id ? submitting_up : submitting_add;
      		...
      		<Form onSubmit={this.handleSubmit}>
      			...
      			<FormItem {...f_i_l} label="课程日期">
	      			{getFieldDecorator('day_of_week', {
        				initialValue: parseInt(lesson.day_of_week || 0),
	      				rules: [{
	      					required: true, message: '课程日期'
	      				}]
	      			})(
      					<Select placeholder="选择日期">
                  {DAY_OF_WEEK.map((item, i) => {return (<Option value={i} key={`d_of_w_${i}`}>{item}</Option>)})}
      					</Select>
	      			)}
      			</FormItem>
