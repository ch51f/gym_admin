...
import style from './Manage.less';

@connect(({manage, loading}) => ({
  loading: loading.effects['manage/getFeedbackList'],

  feedback_data: manage.feedback_data,
}))
export default class Page extends Component {
  ...
  goAddFeedback = () => {
    this.props.history.push('/manage/addFeedback');
  }
  ...
  _reply = () => {
    let {record, open, reply} = this.state;
    let params = {
      worker_id: 1,
      reply,
      item_id: record.id,
    ...
    const paginationProps = {
      ...
      <PageHeaderLayout title="反馈管理">
        ...
        >
          ...
          <Radio.Group value={this.state.open} onChange={this.onChangeOpen}>
