...
export default class Section extends Component {
  ...
  render() {
    const {member} = this.props;
    const {user_name, create_ts, tel, birthday, job, community, extra_info, card_id, gender, teachers} = member;
    const {subscribe_info, checkins, lesson_subscribes} = extra_info;
    const {available_time_begin, worker_name} = subscribe_info;
      ...
      <Row>
        <Col span={16}>
          <Row>
            ...
            <Col span={12}>
              <div className={style.manageCol}>
                有效期：{available_time_begin && available_time_begin > 0 ? getDateStr(available_time_begin) : "-"}
              </div>
            </Col>
            <Col span={12}>     
              <div className={style.manageCol}>
