...
// moment format
export function format(d, format="YYYY-MM-DD") {
...
export function getAge(birthday) {
  const now = new Date();
  const b = setMoment(birthday)
  const n = moment(now);

  return n.year() - b.year();
}

// 日期显示格式
export function getDateStr(str) {
  return format(setMoment(str));
}
// 时间显示格式
...
// 传入元获取分
export function getPriceF(price = 0) {
  return price * 100;
}

export function getPriceY(price = 0) {
  return price / 100;
}
